...
const HomeIcon = (props) => {
	return (
		<a href="/" className="jf-logo" aria-label="Oratechies Home">
	...
	return (
		<AppBar position="sticky" className="jf-appbar">
			<Toolbar className="jf-toolbar">
				...
				<nav className="jf-nav-link jf-nav">
					<Link key="/auth" to="/auth" className="jf-signin">
						<StyledButton startIcon={<LoginIcon />}  variant="outlined">Login</StyledButton>
					</Link>
					<Link key="/auth" to="/auth" className="jf-signin">
						<StyledButton startIcon={<PersonAddAlt1Icon />} variant="contained">Sign Up</StyledButton>
					</Link>
				</nav>

        <div class="jf-nav-drawer">
						<SideDrawer />
				</div>
			</Toolbar>
		</AppBar>
